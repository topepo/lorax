...
* `active_predictors()` gains an `lgb.Booster` method for extracting the set of predictors used in tree splits from LightGBM models. The `tree` argument supports extracting from multiple trees.
...
* `active_predictors()` gains an `xgb.Booster` method for extracting the set of predictors used in tree splits from XGBoost models. The `tree` argument supports extracting from multiple trees.
